...
## CI/CD 策略与 Trade-off 分析

### 当前工作流设计（安全优先）
当前 `.github/workflows/terraform.yml` 仅包含 `fmt`、`validate` 和 `plan` 阶段，**不包含 `apply`**。这是基于以下安全考虑：

#### 优点
1. **零风险自动化**：不会因 CI/CD 错误导致资源意外变更
2. **人工审查强制化**：要求开发者手动审查计划后执行变更
3. **简单可靠**：无需复杂的环境配置或审批流程
4. **符合最小权限原则**：CI 环境仅需读取权限生成计划

#### 缺点
1. **手动操作**：每次变更都需要人工介入执行 `terraform apply`
2. **部署延迟**：无法实现自动化部署流水线
3. **人为错误风险**：手动执行可能输入错误命令
...
**实现方式**：
1. 在 GitHub Actions 页面点击 **Run workflow** 手动触发
2. 依赖已上传的 `tfplan` artifact
...
**实现方式**：
1. GitHub 仓库 Settings → Environments 创建环境
2. 配置审批规则（Required reviewers、等待时间等）
3. 审批人员在 Actions 页面点击 **Review deployment**
...
- ✅ **完全自动化**：推送即部署
- ✅ **零延迟**：变更立即生效
- ❌ **高风险**：任何代码错误都会直接影响生产环境
- ❌ **缺乏审查**：无法阻止错误的配置变更

### tfplan 查看与使用

#### 在 GitHub Actions 中查看：
1. **Actions 页面** → 进入 workflow 运行详情
2. **Artifacts 区域** → 下载 `tfplan` 文件
3. **本地查看命令**：
   ```bash
   terraform show tfplan          # 查看计划摘要
   terraform show -json tfplan   # 查看完整 JSON 格式
   ```
...
| 场景 | 推荐方案 | 理由 |
|------|----------|------|
| **团队初期/测试环境** | 方案一（手动触发） | 平衡安全与灵活性，无需复杂配置 |
| **生产环境/多人协作** | 方案二（环境审批） | 流程化审批，职责分离，审计可追溯 |
| **个人项目/快速迭代** | 方案三（自动应用） | 仅推荐非关键、可随时重建的环境 |

### 决策指南
1. **安全性优先**：当前设计已满足基础安全要求
2. **逐步演进**：可从方案一开始，按需升级到方案二
3. **备份策略**：无论采用哪种方案，都应确保有完整的备份和回滚方案
4. **监控告警**：所有变更都应有对应的监控和告警配置
